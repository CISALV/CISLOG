...
var
  i: Integer;
  IdProduto: Integer;
  bdvProduct, cartProduct: TField;
begin
  inherited;
  IdProduto := dsBDV.Dataset.FieldByName('id').AsInteger;
  ...
  for i := 0 to dsBDV.DataSet.FieldCount - 1 do
  begin
    bdvProduct := dsBDV.Dataset.Fields[i];
    cartProduct := memCarrinho.FindField(bdvProduct.FieldName);
    if Assigned(cartProduct) then
      cartProduct.Value := bdvProduct.Value;
  end;
...
begin
  ...
  SearchBar.DataSource := dsBDV;
  SearchBar.ConfigureFilterFields(['NOME','CNPJ']);


  dsBDV.DataSet := Controller.LoadData;
